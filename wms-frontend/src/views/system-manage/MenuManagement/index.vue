...
const validatorMenuName = (rule, value, callBack) => {
	const reg = /^[a-zA-Z0-9\u4e00-\u9fa5]{2,15}$/;
	if (reg.test(value)) {
		callBack();
	} else {
...
// 自定义菜单顺序校验 请填写1到3位数字！
const validatorMenuSort = (rule, value, callback) => {
	const reg = /^\d{1,3}$/;
	if (reg.test(value)) {
		callback();
	} else {
...
// 表格树形节点点击事件
const haneleTableNode = async (row) => {
